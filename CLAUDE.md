...
- **Frontend**: Cloudflare Pages, proyecto `marc-web`, sirviendo `marc.com.pe` y `www.marc.com.pe`. Un push a `main` dispara `.github/workflows/deploy-frontend.yml`: compila Astro, copia los HTML legacy y assets a `dist/`, y publica con wrangler. El frontend NO está en AWS/S3. El sitio es **100% estático: no hay `functions/`**, y conviene que siga así.
- **Nunca añadir una Pages Function a la ligera**: cualquier archivo en `functions/` hace que **toda** petición del proyecto invoque la Function —assets incluidos, ~55 por visita al home en vez de 0— y eso consume la cuota de Workers & Pages (pasó el 2026-08-17). Si alguna vez hace falta una, acompañarla obligatoriamente de un `public/_routes.json` que excluya `css/`, `js/`, `images/`, `fonts/`, `_astro/` y el favicon.
- **Redirect `www` → apex**: resuelto con una Redirect Rule de zona en Cloudflare (301, creada el 2026-08-18), que corre en el edge sin invocaciones. Si hay que rehacerlo, es en Rules → Redirect Rules; no sirven los `_redirects` de Pages (no soportan reglas por hostname) ni una Pages Function (coste por petición). El editor avisa de que `www` "puede no estar proxied": es un falso positivo con los custom domains de Pages — el registro sí está en naranja, y no hay que crear el registro DNS que sugiere.
