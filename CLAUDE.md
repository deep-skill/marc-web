...
- **Redirect `www` → apex**: pendiente. Debe hacerse como Redirect Rule de zona en Cloudflare (Rules → Redirect Rules), que resuelve en el edge sin invocaciones. No sirven los `_redirects` de Pages (no soportan reglas por hostname) ni una Pages Function (coste por petición).
